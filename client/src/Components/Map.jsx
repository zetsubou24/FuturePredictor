...
const Map = () => {
  ...
  useEffect(() => {
    ...
    window.L.marker(
      [toFixed(40.4212216, 5), toFixed(-3.6286935, 5)],
      {
  ...
  const findNearbyRestaurants = () => {
    ...
    fetch(`https://api.tomtom.com/search/2/poiSearch/restaurant.json?limit=4&lat=${future.Latitude}&lon=${future.Longitude}&radius=1000&categorySet=7315&key=${TomtomKEY}`)
    .then(response => {
      return response.json();
    })
    .then(data => {
      for(var i in data.results){
        console.log(data.results[i].position.lat, data.results[i].position.lon);
        window.L.marker(
  ...
  const findNearbyHospitals = () => {
    const TomtomKEY = "2xrfCdFd2nGVXM5iYXAZpueDTqBfFYIh";
    fetch(`https://api.tomtom.com/search/2/poiSearch/.json?limit=4&lat=${future.Latitude}&lon=${future.Longitude}&radius=10000&categorySet=7321&key=${TomtomKEY}`)
    .then(response => {
      return response.json();
    })
    .then(data => {
  ...
  const findNearbyShoppingCenters = () => {
    const TomtomKEY = "2xrfCdFd2nGVXM5iYXAZpueDTqBfFYIh";
    fetch(`https://api.tomtom.com/search/2/poiSearch/.json?limit=4&lat=${future.Latitude}&lon=${future.Longitude}&radius=10000&categorySet=7373&key=${TomtomKEY}`)
    .then(response => {
      return response.json();
    })
    .then(data => {
      for(var i in data.results){
        ...
        directions.route({
        start: [toFixed(40.4212216, 5), toFixed(-3.6286935, 5)],
        end:[toFixed(data.results[i].position.lat, 5), toFixed(data.results[i].position.lon, 5)]
        },addCustomLayer);
        function addCustomLayer(err, response){
        var customLayer = window.L.mapquest.directionsLayer({
          startMarker: {
  ...
  return (
    <>
      ...
      <Button onClick={() => findNearbyHospitals()}>Find Nearby Hospital</Button>
      <Button onClick={() => findNearbyShoppingCenters()}>Find Nearby Shopping Centers</Button>
